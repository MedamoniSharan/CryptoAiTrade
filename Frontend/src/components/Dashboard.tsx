...
import { LineChart, ArrowUpRight, ArrowDownRight, Clock, Wallet, DollarSign } from 'lucide-react';
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="px-4 py-2 bg-gray-700 rounded-lg">
              ...
              >
                {totalProfit >= 0 ? (
                  <>
                    <ArrowUpRight className="inline-block w-5 h-5 text-green-400" />
                    +${totalProfit.toFixed(2)}
                  </>
                ) : (
                  <>
                    <ArrowDownRight className="inline-block w-5 h-5 text-red-400" />
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            {investments.map((investment) => {
              const isPositiveProfit = parseFloat(investment.expectedProfit) >= 0;
              return (
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            {investment.status === 'pending' ? (
                              ...
                              <div
                                className={`text-lg font-bold ${
                                  isPositiveProfit ? 'text-green-400' : 'text-red-400'
                                }`}
                              >
                                {isPositiveProfit ? (
                                  <>
                                  ...
                                  <>
                                    <ArrowDownRight className="inline-block w-5 h-5 text-red-400" />
                                    -${Math.abs(investment.expectedProfit)}
                                  </>
                                )}
                              </div>
                            )}
